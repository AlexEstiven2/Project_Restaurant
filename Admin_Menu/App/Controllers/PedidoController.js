    ...
    try {
        ...
        const query = `
            ...
            WHERE p.ID_MESA = :idMesa 
              /* CAMBIO CLAVE: Mostramos TODO excepto lo que el admin ya archivó o canceló */
              AND p.ESTADO_PEDIDO NOT IN ('FINALIZADO', 'CANCELADO')
        ...
        console.error("Error en detalle mesa:", error);
...
export const finalizarPedidosMesa = async (req, res) => {
    try {
        const { idMesa } = req.params; 

        // Verificamos que el ID llegue
        if (!idMesa) {
        ...
        const [resultado] = await sequelize.query(
            `UPDATE PEDIDOS 
             ...
             WHERE ID_MESA = ? AND ESTADO_PEDIDO != 'CANCELADO'`,
            { 
                replacements: [idMesa], 
        ...
        res.json({ message: "Pedidos finalizados correctamente", affectedRows: resultado });
    } catch (error) {
        console.error("Error en finalizarPedidosMesa:", error);
